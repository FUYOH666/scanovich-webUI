...
| 14 | Генерация презентаций | Implemented (WOW-3) | **PPTX generation**: `/pptx` или «сделай презентацию / build a deck / в формате pptx» → `task_type=PPTX_GENERATION` → `apps/orchestrator/gpthub_orchestrator/pptx_gen.py` просит **`PPTX_PLAN_MODEL`** (default `gpt-hub-strong` → glm-4.6-357b) сгенерировать JSON slide plan (CoT strip + `raw_decode` для хвостового текста + retry on parse failure), затем `python-pptx` собирает `.pptx` (title slide + bullet slides), файл сохраняется в `/tmp/gpthub_pptx/`, в ответ идёт markdown-ссылка на `GET /v1/files/pptx/{token}`. Опциональные LiteLLM-алиасы для A/B: `gpt-hub-pptx-llama33`, `gpt-hub-pptx-qwen235a22` — см. `scripts/bench_pptx_plan_models.py`. Один `chat.completion` на выход. **57** unit-тестов в `test_pptx_gen.py` + YAML alias test. **Live PASS 2026-04-12**: 7-slide deck, 35 KB, download OK. Трейс: `{short_circuit:"pptx_generation", plan_model, slide_count, byte_size, plan_ms, build_ms, url}`. |
