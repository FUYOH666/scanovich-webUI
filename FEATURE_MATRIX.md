...
|-----|---------|--------|---------------------------|
| 1 | Текстовый чат | Implemented | Open WebUI → orchestrator `/v1/chat/completions` → LiteLLM alias `gpt-hub-turbo/strong` → MWS `mws-gpt-alpha` / `glm-4.6-357b`. Prompt precedence + `X-GPTHub-Trace`. |
...
| 6 | Файлы и ответы по содержимому | Implemented | Orchestrator `ingest/pipeline.py`: PDF через `pypdf`, plus plain-text ingest для `.txt/.md/.json/.yaml/.py/.ts/.go/.sql/...` (≈30 расширений). Текст идёт как `document_text` artifact в system prompt. Тесты: `test_ingest_pdf.py`, `test_ingest_text.py`. |
...
| 9 | Долгосрочная память | Implemented | `apps/orchestrator/gpthub_orchestrator/memory/`: SQLite store (`store.py`), command parser (`commands.py`), MWS `qwen3-embedding-8b` client (`embeddings.py`), высокоуровневый сервис (`service.py`). Orchestrator ловит «запомни X / забудь X / что ты помнишь», сохраняет факты с эмбеддингом, при обычном запросе подмешивает top-K релевантных фактов как system-блок. Тесты: `test_memory_commands.py`, `test_memory_store.py`, `test_memory_service.py` (52 теста). |
| 10 | Автовыбор модели под задачу | Implemented | `classifier.py` (task type + modalities) → `router.py` (role-backed alias chain) → LiteLLM. Решение попадает в `X-GPTHub-Trace`. |
...
|-----|---------|--------|--------------|
| 13 | Deep Research / research-режим | Deferred (wow candidate) | Запланирован как **Expert Council**: новый `task_type=DEEP_RESEARCH`, orchestrator внутри делает fan-out в 3 модели MWS (`strong` + `reasoning` + `doc`), собирает synthesis через `strong`, возвращает один ответ в один chat flow. Архитектурно не ломает «один запрос → один ответ». |
| 14 | Генерация презентаций | Deferred (wow candidate) | Orchestrator-тул: классификатор ловит «сделай презентацию / /pptx» → strong-модель генерирует JSON slide plan → `python-pptx` собирает `.pptx` → возвращается base64 attachment + markdown preview. Зависимость `python-pptx`. |
