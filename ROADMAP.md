...
> **Снимок прогресса:** шаги 1–9, PASS/WARN `demo.sh`, live PPTX (пакет `gpthub_orchestrator/pptx/`, артефакты), hot-fix WebUI PDF и устойчивость JSON-плана — см. **`docs/LIVE_SMOKE.md`**; счётчик тестов — `uv run pytest` (на смежных ветках встречалось **261**).

### Шаг 1 — Live docker smoke (приоритет #1, box: 3 часа)

**Почему первый:** у нас **226+** unit/integration тестов в orchestrator;
...
Остаются операторские сценарии (Row 2, 4, 5, 11) — отдаются шагу 6
(репетиция).
...
### Шаг 7 — WOW-3 PPTX generation (box: 4 часа, условный) — **в коде**, ✅ закрыт

Исторически был kill switch «выбросить, если не успели». Реализация уже в `main`
(`gpthub_orchestrator/pptx/` + тесты модуля). **Статус (2026-04-12):** live
подтверждено: CoT/thinking stripping для MWS glm-4.6; расширенные intent-паттерны;
7-slide deck, скачивание через **`GET /artifacts/pptx/…`**.
...
| PPTX (шаг 7) | JSON plan нестабильный / политика продукта | `pptx_gen_enabled=false` — wow уже в коде; отключаем при сбоях, остальной чат жив |
...
- `[~]` все 12 рядов прогнаны вживую и записаны в `LIVE_SMOKE.md` (авточасть закрыта `demo.sh`; операторские ряды 2/4/5/11 — шаг 6)
- `[x]` каждому ряду в `FEATURE_MATRIX.md` соответствует либо `Implemented`, либо честный `Partial/Deferred` с причиной
- `[x]` `scripts/demo.sh` проходит без ошибок идемпотентно (**PASS=12 FAIL=0**; при необходимости см. WARN по эвристикам шагов 8–9; PPTX live 2026-04-12)
...
- `[x]` PPTX (WOW-3): «сделай презентацию» / «в формате pptx» → markdown + ссылка на **`GET /artifacts/pptx/…`** (скачиваемый .pptx); код + тесты; live 2026-04-12 — см. `LIVE_SMOKE.md`; `[ ]` при смене стенда — повторить запись там
...
### Docs consistency
- `[x]` `README.md`, `ROADMAP.md`, `FEATURE_MATRIX.md`, `CHANGELOG.md`, `docs/NEW_CHAT_HANDOFF_RU.md`, `docs/TEAM_BRIEF_RU.md`, `docs/MODEL_ROUTING_POLICY.md` — синхронны с кодом (**реестр ролей v1** в `data/model_roles.yaml`, **226+** pytest, PPTX — `pptx/` + artifacts)
- `[ ]` git tag `demo-ready` проставлен (шаг 9)
...
1. ✅ **Шаг 1 — Docker smoke** (v3 остановлен, prod поднят; см. tag `smoke-green` в журнале).
2. ✅ **Шаг 2 — Полный ряд smoke** через `demo.sh` без `--skip-wow` (**PASS=12 FAIL=0**;
   WARN по council/PPTX при необходимости — см. вывод скрипта). Операторские ряды 2/4/5/11 — в шаг 6.
3. 🔜 **Шаг 6 — Репетиция** после wow в `main` (нужен оператор в WebUI).
4. 🔜 **Шаг 8 — Запись видео** (после шага 6).
