...
| 14 | Генерация презентаций | `task_type=PPTX_GENERATION` → `pptx_gen.py`: JSON slide plan via `gpt-hub-strong` (retry) → `python-pptx` → download link `GET /v1/files/pptx/{token}` | `[x]` WOW-3: 52 теста, FEATURE_MATRIX → `Implemented (WOW-3)` |
...
> - ✅ Step 2 (полный P0 smoke через `demo.sh` без `--skip-wow`) — PASS=12 FAIL=0 WARN=1. Остаётся ручной WebUI-проход (Step 6).
...
> - ✅ Step 7 (WOW-3 PPTX) — код + 52 теста в `main`; **234 теста** total.
> - 🔜 Step 8 (запись видео) — оператор.
> - 🔜 Step 9 (финальная сверка + git tag `demo-ready`).
>
> **Параллельный hot-fix 2026-04-12:** Row 6 PDF upload крашился в WebUI с
...
> retry, чтобы окончательно зафиксировать Row 6 как `Implemented` в matrix.

### Шаг 1 — Live docker smoke (приоритет #1, box: 3 часа) — ✅ ЗАКРЫТО

**Почему первый:** у нас **234** unit/integration теста в orchestrator;
...
без `--skip-wow` — PASS=12 FAIL=0 WARN=1 (WARN — Row 14, ещё не сделан).
Остаются операторские сценарии (Row 2, 4, 5, 6, 11) — отдаются Step 6
(репетиция).
...
### Шаг 4 — Row 7 Tavily toggle (box: 30 минут) — 🔜 СЛЕДУЮЩИЙ
...
### Шаг 7 — WOW-3 PPTX generation (box: 4 часа, условный) — 🔜 ЗАПЛАНИРОВАН

**Делаем только если** Council уложился и осталось 4+ часов до дедлайна.
Иначе **выбрасываем** без сожалений.
...
| PPTX (шаг 7) | JSON plan нестабильный / зависимость не ставится | Выбрасываем без замены |
...
- `[x]` `scripts/demo.sh` проходит без ошибок идемпотентно (PASS=12 FAIL=0 WARN=1; единственный WARN — Row 14 PPTX до Step 7)
...
- `[ ]` (wow-3, если есть) PPTX: «сделай презентацию» возвращает скачиваемый .pptx — Step 7
...
### Docs consistency
- `[x]` `README.md`, `ROADMAP.md`, `FEATURE_MATRIX.md`, `CHANGELOG.md`, `docs/NEW_CHAT_HANDOFF_RU.md`, `docs/TEAM_BRIEF_RU.md` — синхронны на 2026-04-12
...
2. ✅ **Шаг 2 — Полный ряд smoke** через `demo.sh` без `--skip-wow`
   (PASS=12 FAIL=0 WARN=1; WARN — Row 14 PPTX). Operator-ряды (2/4/5/6/11)
   собираем в Step 6.
...
- [ ] [markitdown](https://github.com/microsoft/markitdown) — DOCX, XLSX,
  PPTX → markdown ingestion. Заменяет pypdf + custom reader единым
  конвертером. Добавлять, когда пользователи реально грузят DOCX.
