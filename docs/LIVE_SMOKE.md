...
## 2026-04-15 — Row 6 / Row 4: PPTX + голос + PDF (YC VM, см. `logs.txt`)
...
- **Result:** **PARTIAL**
  - **A:** **OK** — презентация разобрана, ответ по смыслу слайдов; **аудиозапрос обработан корректно**.
  - **B:** **FAIL (продуктовый край)** — после PPTX **PDF фактически проигнорирован**: ассистент **продолжил опираться на презентацию**, содержимое PDF в ответе **не задействовано** (ожидаемо при одном user-turn с несколькими вложениями, если ingest последнего сообщения не очередит несколько `files` или контекст модели «прилип» к первому артефакту — зафиксировать для бэклога).
  - **C:** **OK** — в **отдельном** чате **PDF обработан корректно**.
- **Trace highlights:** для успешных PPTX/PDF — в оркестраторе ожидаются **`ingest_peek`** с ненулевым **`open_webui_disk`** (или эквивалент) и артефакты **`document_text`** / PDF extract в system-блоке; для **B** — визуально верный ответ только по PPTX.
- **Notes:** Зафиксировано для **FEATURE_MATRIX** ряд 6: одиночные вложения на прод-стеке закрывают сценарий; **мульти-вложение в одном сообщении (PPTX затем PDF)** — отдельная доработка (очередь ingest по всем `files` или явный сброс контекста). Сырые логи — **`ресурсы/YandexCloud/logs.txt`**. Made-with: Cursor.
