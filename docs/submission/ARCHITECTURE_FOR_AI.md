...
Этот файл **UTF-8 Markdown** дублирует смысл
[`GPTHub_architecture_submission.pdf`](GPTHub_architecture_submission.pdf), но в
формате, который **текстовые и RAG-агенты** читают надёжно: нормальная кириллица,
структурированные заголовки, **исходники Mermaid** и **текстовые пересказы** схем
(страницы 2–3 PDF — растровые картинки, из них нельзя извлечь граф без vision/OCR).

Для **человеческой** формы сдачи по-прежнему используйте один **PDF**, если
площадка требует только `.pdf`. Если проверка **гибридная** (PDF + репозиторий),
укажите жюри путь к этому файлу или приложите его вторым артефактом, если правила
позволяют.
...
## Почему PDF хуже для «слепого» текстового ИИ

- Первая страница PDF собрана через Matplotlib: **извлечение текста** типовыми
  библиотеками часто даёт **нечитаемую кодировку** вместо кириллицы.
- Вторые страницы — **PNG внутри PDF**: семантики рёбер и подписей нет, пока
  модель явно не запускает **vision** по растру.

Итог: для автоматической проверки и индексации держите канон в **этом `.md`** и в
`.mmd`; PDF оставьте для людей и для поля формы «один файл».
