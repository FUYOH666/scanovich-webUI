...
**261 тест в orchestrator, все зелёные.**
...
- **Row 6 Files** — ingest PDF + **DOCX/XLSX/PPTX** (via markitdown) +
  plain text (≈30 расширений) в orchestrator.
...
- **Row 9 Long-term memory** — `memory/`: SQLite store + MWS
  ...
  что ты помнишь». Memory short-circuit + retrieval injection. Покрыто
  `test_memory_*.py` (~30 тестов); полный suite — **261 passed**.
  **Live:** recall ок; **save** иногда **ReadTimeout** на MWS embeddings — см. `LIVE_SMOKE.md`, `memory_embedding_timeout_seconds`.
- **Row 10 Автовыбор модели** — classifier + router + `model_roles.yaml`:
  doc/summary → **`gpt-hub-doc`** (fallback turbo); code → **`gpt-hub-reasoning-or`**
  (fallback turbo); простой чат → turbo (alpha). **Live trace в `demo.sh`**;
...
- **Row 14 PPTX (WOW-3)** — **Implemented** ✅. `pptx_gen.py`:
  JSON slide plan via `gpt-hub-strong` (with `<think>` CoT stripping +
  retry on parse failure) → `python-pptx` → download link
  `GET /v1/files/pptx/{token}`. 57 unit-тестов. **Live PASS 2026-04-12:
...
- `scripts/demo.sh` без `--skip-wow` — базовый ожидаемый итог **PASS=12 FAIL=0**;
  **WARN>0** бывает только если эвристики шагов 8–9 не находят council/PPTX в теле ответа (перепроверить вывод). Журнал **2026-04-11 WARN=1 из‑за PPTX до Step 7** — **устарел** (WOW-3 в main, live PPTX 2026-04-12).
...
2. **Шаг 2 ✅ В ОСНОВНОМ ЗАКРЫТО — P0 smoke.** `demo.sh` без `--skip-wow`
   — **PASS=12 FAIL=0** (журнал 2026-04-11 11:46: WARN=1 **до** WOW-3 PPTX;
   после деплоя PPTX — ожидать **WARN=0**, иначе см. вывод шагов 8–9). Остаются operator-сценарии (Шаг 6).
...
7. **Шаг 7 ✅ ЗАКРЫТО — WOW-3 PPTX** — `pptx_gen.py` + 57 тестов в `main`. **Live PASS 2026-04-12.**
...
3. **Шаг 7 ✅ — WOW-3 PPTX** — `pptx_gen.py` + 57 тестов в `main`. **Live 2026-04-12.**
...
2. **Шаг 2 ✅ — Полный P0 smoke.** `demo.sh`: **PASS=12 FAIL=0**; журнал
   `docs/LIVE_SMOKE.md` (2026-04-11: WARN=1 до PPTX — исторический снимок).
...
4. ✅ **WOW-3 PPTX** (трек A, шаг 7) — код + 57 тестов, **live PASS 2026-04-12**.
