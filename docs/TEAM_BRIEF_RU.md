...
Коротко: **почти да, осталось 2 ряда и живой docker smoke.**
...
- live docker smoke + voice demo прогон;
- wow-features: Expert Council, PPTX;
...
4. **Шаг 5: WOW-1 Expert Council** в отдельной ветке, жёсткий box 6 часов.
   Kill switch: не взлетел — откат до `smoke-green`, Council в backlog.
5. **Шаг 6: репетиция демо-сценария** до записи видео.
6. **Шаг 7: WOW-3 PPTX** — **условный**, только если Council уложился.
...
1. **Шаг 5 — WOW-1 Expert Council** в ветке `wow/expert-council`.
   Box: 6 часов. Kill switch: если не взлетело end-to-end — `git reset
   --hard smoke-green`, Council в backlog.
2. **Шаг 4 — Row 7 Tavily toggle** (15 минут).
3. **Шаг 7 — WOW-3 PPTX** в ветке `wow/pptx`. **Условно** — только если
   Council уложился и осталось 4+ часов. Иначе выбрасываем.

Правило гигиены: wow-ветки вливаются в main **только** после зелёного
прогона через WebUI, не раньше.
...
2. **Submission артефакты** (трек C, шаг 3) — параллельно шагу 1, если
   есть второй человек.
3. **WOW-1 Expert Council** (трек A, шаг 5) — только после smoke-green.
4. **Row 7 Tavily** (трек A, шаг 4) — 15 минут в любом удобном окне.
5. **Репетиция** (трек B, шаг 6).
6. **WOW-3 PPTX** (трек A, шаг 7) — **условно**.
7. **Видео + финализация** (треки B и C, шаги 8–9).

Если людей совсем мало — делаем только шаги 1, 2, 3, 5 (репетиция),
8, 9. WOW-компоненты выбрасываем без сожалений, защищаемся на чистом
baseline + mixed input + X-GPTHub-Trace.
