...
- **Row 13 Expert Council (WOW-1)** — в `main` (merge `9393d30`): `council.py`,
  ...
  2026-04-11.
- **Row 14 PPTX (WOW-3)** — пакет `gpthub_orchestrator/pptx/`: `task_type=pptx`,
  план (parallel slide agents или монолит + retry), `python-pptx`, ответ с markdown
  + **`GET /artifacts/pptx/{id}?token=…`**. **36** тестов; live — `LIVE_SMOKE.md`.

### Ещё не закрыто

- **Row 7 Web search** — включить `ENABLE_WEB_SEARCH=true` + `TAVILY_API_KEY` в env Open WebUI.
...
- Старый стек `gpthub-v3-*` при необходимости остановить (порты 3000/4000/8089).
- Живые прогоны фиксировать в `docs/LIVE_SMOKE.md` (в т.ч. PPTX, council, `demo.sh`).
...
6. **Шаг 7: WOW-3 PPTX** — **готово** в `main` (пакет `pptx/`).
...
- **один цельный сценарий**: PDF + скрин + аудио + ссылка → «запомни мои
  ...
  «сделай презентацию» (PPTX → ссылка на `.pptx`);
...
2. **Шаг 4 — Row 7 Tavily toggle** (env WebUI).
3. **Шаг 7 — WOW-3 PPTX** — **готово** в `main` (`gpthub_orchestrator/pptx/`).
...
2. **Submission артефакты** (трек C, шаг 3) — параллельно шагу 1, если
   есть второй человек.
3. **Row 7 Tavily** (трек A, шаг 4) — env WebUI.
4. **Репетиция** (трек B, шаг 6).
5. **Видео + финализация** (треки B и C, шаги 8–9).
